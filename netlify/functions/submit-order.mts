...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    // An account is now required to place an order. The Order page only shows the
    // form after confirming login, but we verify server-side too — never trust the
    // client alone.
    ...
    if (!userId) {
      return new Response(
        JSON.stringify({ error: "Please log in to place an order." }),
        { status: 401 }
      );
    }
    ...
    const priceAfterCoupon = appliedDiscountPercent
      ? Math.round(subtotal * (1 - appliedDiscountPercent / 100))
      : subtotal;

    // Loyalty points redemption — always re-checked against the real, server-computed
    // balance. Never trust how many points the browser says to redeem.
    const requestedPoints = parseInt((formData.get("points_redeemed") as string) || "0", 10) || 0;
    let pointsToRedeem = 0;
    if (requestedPoints > 0 && userId) {
      const realBalance = await getLoyaltyBalance(SUPABASE_URL, SERVICE_KEY, userId);
      pointsToRedeem = Math.min(requestedPoints, realBalance, priceAfterCoupon);
      if (pointsToRedeem < 0) pointsToRedeem = 0;
    }
    const totalPrice = priceAfterCoupon - pointsToRedeem;
    ...
    const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/orders`, {
      ...
      body: JSON.stringify({
        ...
        points_discount_amount: pointsToRedeem,
      }),
    });
